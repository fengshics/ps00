;; The first three lines of this file were inserted by DrRacket. They record metadata
;; about the language level of this file in a form that our tools can easily process.
#reader(lib "htdp-beginner-reader.ss" "lang")((modname set00:4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
;Ex 4 of lesson 0.4 by Feng Shi
(require rackunit)


; tip : NonNegNumber Number[0.0,1.0] -> Number
; GIVEN: the amount of the bill in dollars and the
; percentage of tip
; RETURNS: the amount of the tip in dollars. 
; false when percentage out of range or bill is negtive
; Examples:
; (tip 10 0.15)  => 1.5
; (tip 20 0.17)  => 3.4
; (tip -10 0)    => false
(define (tip bill perc)
  (cond
    [(and (>= bill 0) (and (>= perc 0.0) (<= perc 1.0))) (* bill perc)]
    [else false]))

;test
(define begin-for-test
  (check-equal?
   (list 1.5 3.4 false false)
   (list (tip 10 0.15) (tip 20 0.17) (tip 30 8) (tip -10 0))
   "tip did not return the correct tip"))


